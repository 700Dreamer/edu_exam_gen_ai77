    ...
    try:
        ...
        if mode == "Exams":
            for q in data.get("questions", []):
                ...
                if is_drawing_question:
                    ...
                    if tikz:
                        ...
                        elif "\\begin" in str(tikz):
                            if "<script" in str(tikz).lower():
                                parsed_html += f"  <div class='ill-box' style='text-align:center; padding:15px; width:100%;'>{tikz}</div>"
                            else:
                                parsed_html += f"  <div class='ill-box' style='text-align:center; padding:15px; width:100%;'><script type='text/tikz'>{tikz}</script></div>"
                        else:
                            parsed_html += f"  <div class='ill-box' style='text-align:center; padding:15px; width:100%;'>{tikz}</div>"
            ...
            for s in data.get("sections", []):
                ...
                if tikz:
                    if "\\begin" in str(tikz):
                        if "<script" in str(tikz).lower():
                            parsed_html += f"  <div style='text-align:center; padding: 15px;'>{tikz}</div>"
                        else:
                            parsed_html += f"  <div style='text-align:center; padding: 15px;'><script type='text/tikz'>{tikz}</script></div>"
                    else:
                        parsed_html += f"  <div style='text-align:center; padding: 15px;'>{tikz}</div>"
                parsed_html += f"</div>"
...
.tmpl-uneb.page {{ border: none !important; box-shadow: none !important; border-radius: 0 !important; }}
.tmpl-uneb.page::before {{ border: 6px double #000 !important; }}
...
@media print {{ 
  ...
  .page::before {{ border-color: #000 !important; }}
}}
</style>
</head>
<body>
...
document.addEventListener("DOMContentLoaded", function() {{
  ...
  function paginateContent() {{
    ...
    // Reduced to 920 for extra safety against accidental overflows (blank pages)
  ...
  window.addEventListener('load', () => setTimeout(paginateContent, 300));
  setTimeout(paginateContent, 1000); // Fallback for dynamic content
